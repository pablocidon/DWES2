...
require 'confUsuarios.php';//Llamamos a la libreria
$entrada=false;
$usuario=$_SERVER['PHP_AUTH_USER'];//Alamacenamos el usuario en una variable.
$password=hash(sha256,$_SERVER['PHP_AUTH_PW']);//Alamacenamos la contraseña encriptandola.
try{
    ...
    $consulta = $miDB->query("SELECT * FROM Usuarios WHERE CodUsuario=$usuario AND Password=$password");
    $consulta->execute();
