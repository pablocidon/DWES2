...
if(filter_has_var(INPUT_POST, 'aceptar')) {
    ...
    try {
        ...
        if ($consulta->rowCount() == 1) {//Si la consulta devuelve un resultado
            ...
            if(isset($_COOKIE['ultimaConexion'])){//Si la cookie no está vacía asignamos el valor a la sesión
                $_SESSION['ultimaConexion']=$_COOKIE['ultimaConexion'];
            }
            setcookie("ultimaConexion",date("d-m-Y g:i:s"),time()+172800000);//Establecemos la cookie para que guarde la hora de conexión
